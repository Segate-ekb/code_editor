...
// Переключает режим отладки (панель отладки + нижнее табло).
//
&НаКлиенте
Процедура ПереключитьРежимОтладки(Команда)
	
	Элементы.ПереключитьРежимОтладки.Пометка = НЕ Элементы.ПереключитьРежимОтладки.Пометка;
	конс_ПодключаемаяКонсольКлиент.РежимОтладки(ЭтаФорма, Элементы.ПереключитьРежимОтладки.Пометка);
	
	Если Элементы.ПереключитьРежимОтладки.Пометка Тогда
		ПоказатьОповещениеПользователя("Режим отладки", , "Панель отладки включена");
	Иначе
		ПоказатьОповещениеПользователя("Режим отладки", , "Панель отладки отключена");
	КонецЕсли;
