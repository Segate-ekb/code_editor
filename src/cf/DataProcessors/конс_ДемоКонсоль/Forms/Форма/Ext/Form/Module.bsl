...
Процедура ВнешнееВыполнениеКода(Команда)
	...
	// Можно передать ФильтрПеременных (4-й параметр) для анализа только конкретных переменных
...
ФильтрПеременных.Добавить("Сумма");
	...
	ДанныеКода = конс_ПодключаемаяКонсольКлиент.ПолучитьКодДляВнешнегоВыполнения(
		ЭтаФорма,
		ПредопределенноеЗначение("Перечисление.конс_РежимПоказаПеременных.ВТабло"), ,ФильтрПеременных);
