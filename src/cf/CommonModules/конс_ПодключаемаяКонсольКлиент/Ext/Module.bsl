...
//
Функция ИзвлечьОписаниеОтНачалаДоИндекса(СтрокиСтека, КонечныйИндекс)
...
//   Строка - описание ошибки
//
Функция ИзвлечьОписаниеОшибкиИзСтрок(СтрокиСтека, ИндексНачала)
...
Процедура ПослеКопированияИндексногоФайла(СкопированныйФайл, КонтекстИнициализации) Экспорт
	...
	Иначе
		// Fallback: если элемент не привязан к реквизиту, пробуем конс_HTML
...
Функция ПолучитьКонтекстПоля(Форма, ЭлементИлиИмя)
	...
	// Получаем путь к данным из элемента
	Если Результат.Элемент <> Неопределено Тогда
		Попытка
			Результат.ПутьКДанным = Результат.Элемент.ПутьКДанным;
		Исключение
			// Нет пути к данным - игнорируем
		КонецПопытки;
	КонецЕсли;
...
// Если Неопределено - берёт из реквизита формы конс_ИмяПоляHTML или возвращает "HTML".
//
...
Функция ИзвлечьПозициюИзМодульнойОшибки(ОписаниеОшибки)
	...
	// Извлекаем координаты между ( и )
	Координаты = Сред(ОстатокСтроки, 2, ПозицияЗакрытияСкобки - 2);
	Части = СтрРазделить(Координаты, ",");
