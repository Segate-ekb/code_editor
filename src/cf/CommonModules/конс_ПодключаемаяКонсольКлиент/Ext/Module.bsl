...
//
Функция ПолучитьКодДляВнешнегоВыполнения(Форма, ИмяПоляHTML = Неопределено) Экспорт
	...
	Если Форма.конс_РежимПоказаПеременных > 0 Тогда
		...
		Если Переменные.length > 0 Тогда
			Для Индекс = 0 По Переменные.length - 1 Цикл
				МассивИмен.Добавить(Переменные["" + Индекс + ""]);
			КонецЦикла;
		КонецЕсли;
...
Процедура ОбработатьРезультатВнешнегоВыполнения(Форма, ДанныеКода, ИнформацияОбОшибке, ИмяПоляHTML = Неопределено) Экспорт
	...
	Результат = конс_ПодключаемаяКонсольВызовСервера.ОбработатьРезультатВнешнегоВыполнения(
		...
		Форма.конс_РежимПоказаПеременных);
...
Функция РазобратьИнформациюОбОшибке(ИнфоОшибки) Экспорт
	...
	// Извлекаем позицию из описания только для выполняемого кода {(строка,колонка)}:...
	ПозицияИзОписания = ИзвлечьПозициюВыполняемогоКода(ИнфоОшибки.Описание);
...
// ВАЖНО: Эта функция работает ТОЛЬКО для формата выполняемого кода {(номер)}!
// Для формата {Модуль(номер)} используйте ИнформацияОбОшибке.НомерСтроки
//
Функция ИзвлечьПозициюВыполняемогоКода(ОписаниеОшибки)
	...
	Если Не СтрНачинаетсяС(ОписаниеОшибки, "{(") Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Координаты = Сред(ОписаниеОшибки, 3, ПозицияЗакрытия - 3);
