...
// Получает текст из редактора с инлайнингом процедур и функций.
// 
// Код, содержащий определения Процедура/Функция и их вызовы,
// преобразуется в плоский код, пригодный для выполнения через Выполнить().
// 
// Параметры:
//   Форма - ФормаКлиентскогоПриложения
//   ИмяПоляHTML - Строка
//
// Возвращаемое значение:
//   Строка - инлайненный текст из редактора
//
Функция ПолучитьИнлайнТекст(Форма, ИмяПоляHTML = Неопределено) Экспорт
	
	Текст = ПолучитьТекст(Форма, ИмяПоляHTML);
	Возврат конс_Инлайнер.Инлайн(Текст);
