...
// Включает или отключает панель отладки и нижнее табло целиком.
// При отключении завершает активную сессию отладки, скрывает тулбар,
// область точек останова и нижнюю панель (переменные/стек вызовов/ошибки/сообщения).
// 
// Параметры:
//   Форма - ФормаКлиентскогоПриложения
//   Включен - Булево - Истина — включить, Ложь — выключить
//   ИмяПоляHTML - Строка - имя поля HTML (необязательно)
//
Процедура РежимОтладки(Форма, Включен, ИмяПоляHTML = Неопределено) Экспорт
	
	Имя = ПолучитьИмяПоляHTML(Форма, ИмяПоляHTML);
	View(Форма, Имя).setDebugPanels(Включен);
