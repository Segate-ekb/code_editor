...
//
Функция ПолучитьТекст(Форма, ИмяПоляHTML = "ПолеHTML") Экспорт
	
	Возврат View(Форма, ИмяПоляHTML).getText();
...
//
Процедура УстановитьТекст(Форма, Текст, Позиция = 0, ИмяПоляHTML = "ПолеHTML", УчитыватьОтступПервойСтроки = Ложь) Экспорт
	
	View(Форма, ИмяПоляHTML).setText(Текст, Позиция, УчитыватьОтступПервойСтроки);
...
//   ИмяПоляHTML - Строка
//
Процедура ОчиститьТекст(Форма, ИмяПоляHTML = "ПолеHTML") Экспорт
	
	View(Форма, ИмяПоляHTML).eraseText();
...
//   ИмяПоляHTML - Строка
//
Процедура ВставитьТекст(Форма, Текст, ИмяПоляHTML = "ПолеHTML") Экспорт
	
	View(Форма, ИмяПоляHTML).insertText(Текст);
...
//
Функция ПолучитьВыделенныйТекст(Форма, ИмяПоляHTML = "ПолеHTML") Экспорт
	
	Возврат View(Форма, ИмяПоляHTML).getSelectedText();
...
Процедура ЗагрузитьПользовательскийКонтекст(Форма, ИмяПоляHTML = Неопределено) Экспорт
	...
	// Загружаем переменные (данные хранятся в реквизите формы конс_JSONПеременных)
	Попытка
		JSONПеременных = Форма.конс_JSONПеременных;
		Если ЗначениеЗаполнено(JSONПеременных) Тогда
			JSONПеременных = "{""customObjects"":" + JSONПеременных + "}";
			Сообщить("[DEBUG] Загрузка переменных: " + JSONПеременных);
			View(Форма, ИмяПоля).updateMetadata(JSONПеременных);
		КонецЕсли;
	Исключение
		// Реквизит не существует или ошибка загрузки
	КонецПопытки;
	
	// Загружаем функции (данные хранятся в реквизите формы конс_JSONФункций)
	Попытка
		JSONФункций = Форма.конс_JSONФункций;
		Если ЗначениеЗаполнено(JSONФункций) Тогда
			JSONФункций = "{""customFunctions"":" + JSONФункций + "}";
			Сообщить("[DEBUG] Загрузка функций: " + JSONФункций);
	...
	// Загружаем сниппеты (данные хранятся в реквизите формы конс_JSONСниппетов)
	Попытка
		JSONСниппетов = Форма.конс_JSONСниппетов;
		Если ЗначениеЗаполнено(JSONСниппетов) Тогда
			JSONСниппетов = "{""snippets"":" + JSONСниппетов + "}";
			Сообщить("[DEBUG] Загрузка сниппетов: " + JSONСниппетов);
...
//   ИмяПоляHTML - Строка
//
Процедура ВыполнитьКод(Форма, НаСервере = Истина, ИмяПоляHTML = Неопределено) Экспорт
	
	ИмяПоля = ПолучитьИмяПоляHTML(Форма, ИмяПоляHTML);
	Код = ПолучитьТекстСКодомПросмотраПеременных(Форма, ИмяПоля);
	
	Если НаСервере Тогда
		Результат = конс_ПодключаемаяКонсольВызовСервера.ВыполнитьКодНаСервере(Код, Форма.конс_РежимПоказаПеременных);
	Иначе
		Результат = ВыполнитьКодНаКлиенте(Код, Форма.конс_РежимПоказаПеременных);
	КонецЕсли;
...
//   ИмяПоляHTML - Строка
//
Процедура ПрименитьНастройки(Форма, Настройки, ИмяПоляHTML = "ПолеHTML") Экспорт
	...
	// Режим показа переменных
	Если Настройки.Свойство("ПоказыватьПеременные") Тогда
...
//
Процедура ЗагрузитьСниппетыИзФайла(Форма, ПутьКФайлу, ИмяПоляHTML = "ПолеHTML") Экспорт
...
//   }
//
Функция ЗагрузитьСниппетыИзJSON(Форма, JSONСниппетов, ИмяПоляHTML = "ПолеHTML") Экспорт
	
	Попытка
		Результат = View(Форма, ИмяПоляHTML).updateSnippets(JSONСниппетов);
		Возврат (ТипЗнч(Результат) = Тип("Булево") И Результат);
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
//   ИмяПоляHTML - Строка
//
Процедура УстановитьТему(Форма, ИмяТемы, ИмяПоляHTML = "ПолеHTML") Экспорт
	
	ТемаРедактора = СоответствиеТем().Получить(ИмяТемы);
	Если ТемаРедактора <> Неопределено Тогда
		View(Форма, ИмяПоляHTML).setTheme(ТемаРедактора);
	КонецЕсли;
...
//   ИмяПоляHTML - Строка
//
Процедура УстановитьРежимТолькоЧтение(Форма, ТолькоЧтение, ИмяПоляHTML = "ПолеHTML") Экспорт
	
	View(Форма, ИмяПоляHTML).setReadOnly(ТолькоЧтение);
...
//
Процедура УстановитьРежимЯзыка(Форма, РежимЯзыка, ИмяПоляHTML = "ПолеHTML") Экспорт
	
	View(Форма, ИмяПоляHTML).setLanguageMode(РежимЯзыка);
...
Функция РазобратьИнформациюОбОшибке(ИнфоОшибки) Экспорт
	...
	// Извлекаем позицию из описания (формат: {Модуль(строка,колонка)}: Текст)
	ПозицияИзОписания = ИзвлечьПозициюИзОписанияКлиент(ИнфоОшибки.Описание);
	
	Результат = Новый Структура;
	Результат.Вставить("Описание", ИнфоОшибки.Описание);
	
	// Используем номер строки из свойства, если он есть, иначе из описания
	Если ИнфоОшибки.НомерСтроки > 0 Тогда
		Результат.Вставить("НомерСтроки", ИнфоОшибки.НомерСтроки);
	Иначе
		Результат.Вставить("НомерСтроки", ПозицияИзОписания.НомерСтроки);
	КонецЕсли;
	
	Результат.Вставить("НомерКолонки", ПозицияИзОписания.НомерКолонки);
	Результат.Вставить("ИмяМодуля", ИнфоОшибки.ИмяМодуля);
	Результат.Вставить("ИсходнаяСтрока", ИнфоОшибки.ИсходнаяСтрока);
	
	// Рекурсивно разбираем причину
	Если ИнфоОшибки.Причина <> Неопределено Тогда
		Результат.Вставить("Причина", РазобратьИнформациюОбОшибке(ИнфоОшибки.Причина));
	Иначе
...
Процедура ПоказатьОшибкуВыполнения(Форма, ОписаниеОшибки, ПоказыватьПричину = Истина, ИмяПоляHTML = Неопределено) Экспорт
	...
	// Определяем какую ошибку показывать
	ОшибкаДляПоказа = ОписаниеОшибки;
	Если ПоказыватьПричину И ОписаниеОшибки.Причина <> Неопределено Тогда
		ОшибкаДляПоказа = ПолучитьКорневуюПричину(ОписаниеОшибки);
	КонецЕсли;
	
	// Показываем сообщение
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = ОшибкаДляПоказа.Описание;
	Сообщение.Сообщить();
	
	// Помечаем строку в редакторе
	НомерСтроки = ОшибкаДляПоказа.НомерСтроки;
	НомерКолонки = ОшибкаДляПоказа.НомерКолонки;
...
Процедура ОбработчикOnReady(Форма, ИмяПоляHTML = "HTML") Экспорт
	...
	Если Форма.конс_ИсходникиЗагружены Тогда
...
Процедура ПослеПроверкиСуществованияКаталога(Существует, КонтекстИнициализации) Экспорт
	...
	Если Существует Тогда
		...
		ВерсияОбработки = КонтекстИнициализации.Форма.конс_ВерсияОбработки;
...
Процедура ПослеКопированияИндексногоФайла(СкопированныйФайл, КонтекстИнициализации) Экспорт
	...
	Форма.конс_HTML = СкопированныйФайл;
...
Процедура ПослеУдаленияВременныхФайлов(КонтекстИнициализации) Экспорт
	...
	ДанныеМакета = ПолучитьИзВременногоХранилища(КонтекстИнициализации.Форма.конс_АдресМакета);
...
Функция View(Форма, ИмяПоляHTML)
	
	Возврат Форма.Элементы[ИмяПоляHTML].Документ.defaultView;
...
Функция ВебДокументДоступен(Форма, ИмяПоляHTML)
	
	Попытка
		Возврат Форма.Элементы[ИмяПоляHTML].Документ.defaultView <> Неопределено;
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
Процедура ИнициализироватьРедакторКлиент(Форма, ИмяПоляHTML)
	...
	Попытка
		АдресНастроек = Форма.конс_АдресНастроек;
		Если ЗначениеЗаполнено(АдресНастроек) Тогда
			Настройки = ПолучитьИзВременногоХранилища(АдресНастроек);
		КонецЕсли;
	Исключение
		// Реквизит конс_АдресНастроек отсутствует на форме - это нормально
	КонецПопытки;
...
Процедура ОбновитьСписокОбщихМодулей(Форма, ИмяПоляHTML)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Форма.конс_АдресОбщихМодулей) Тогда
			// Получаем JSON строку из хранилища
			ОбщиеМодулиJSON = ПолучитьИзВременногоХранилища(Форма.конс_АдресОбщихМодулей);
...
Функция ЗагрузитьМодульИзИсходников(Форма, ПараметрыЗапроса, ИмяПоляHTML)
	...
	Сообщить("[DEBUG] КаталогИсходногоКода: '" + КаталогИсходногоКода + "'");
	
	Если Не ЗначениеЗаполнено(КаталогИсходногоКода) Тогда
		Сообщить("[DEBUG] Путь к исходникам не задан!");
		Возврат Ложь;
	КонецЕсли;
	...
	Сообщить("[DEBUG] Количество частей запроса: " + КоличествоЧастей);
...
Функция ЗагрузитьОбщийМодульИзИсходников(Форма, КаталогИсходногоКода, ИмяМодуля, ИмяПоляHTML)
	...
	Сообщить("[DEBUG] Загрузка общего модуля: " + ИмяМодуля);
	...
	Если РезультатЗагрузки = Неопределено Тогда
		Сообщить("[DEBUG] Не удалось прочитать текст общего модуля: " + ИмяМодуля);
		Возврат Ложь;
	КонецЕсли;
	
	Сообщить("[DEBUG] Текст модуля прочитан, длина: " + СтрДлина(РезультатЗагрузки.ТекстМодуля));
	...
	Попытка
		...
		Сообщить("[DEBUG] parseCommonModule вернул: " + РезультатПарсинга);
		Возврат Истина;
	Исключение
		Сообщить("[DEBUG] Ошибка parseCommonModule: " + ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
...
Функция ЗагрузитьМодульМенеджераИлиОбъектаИзИсходников(Форма, КаталогИсходногоКода, ТипМодуля, КоллекцияМетаданных, ИмяОбъекта, ИмяПоляHTML)
	...
	Сообщить("[DEBUG] Загрузка модуля " + ТипМодуля + ": " + КоллекцияМетаданных + "." + ИмяОбъекта);
	...
	Если РезультатЗагрузки = Неопределено Тогда
		Сообщить("[DEBUG] Не удалось прочитать текст модуля объекта!");
		Возврат Ложь;
	КонецЕсли;
	
	Сообщить("[DEBUG] Текст модуля прочитан, длина: " + СтрДлина(РезультатЗагрузки.ТекстМодуля));
	Сообщить("[DEBUG] АдресЗагрузки: " + РезультатЗагрузки.АдресЗагрузки);
	Сообщить("[DEBUG] Первые 200 символов текста: " + Лев(РезультатЗагрузки.ТекстМодуля, 200));
	...
	Попытка
		...
		Сообщить("[DEBUG] parseMetadataModule вернул: " + РезультатПарсинга);
		Возврат Истина;
	Исключение
		Сообщить("[DEBUG] Ошибка parseMetadataModule: " + ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
...
Процедура ОбработатьСобытиеРаскрытияПеременной(Форма, Параметры, ИмяПоляHTML)
	...
	// Получаем адрес хранения переменных из реквизита формы
	АдресХраненияПеременных = "";
	Попытка
		АдресХраненияПеременных = Форма.конс_АдресХраненияПеременных;
	Исключение
		Возврат;
	КонецПопытки;
...
Функция ПолучитьТекстСКодомПросмотраПеременных(Форма, ИмяПоляHTML)
	
	Код = ПолучитьТекст(Форма, ИмяПоляHTML);
	
	Если Форма.конс_РежимПоказаПеременных > 0 Тогда
...
Процедура ОбработатьУспешноеВыполнениеПубличный(Форма, Результат, ИмяПоляHTML)
	...
	Попытка
		Если Результат.Свойство("АдресХраненияПеременных") Тогда
			Форма.конс_АдресХраненияПеременных = Результат.АдресХраненияПеременных;
		КонецЕсли;
	Исключение
		// Реквизит отсутствует - пропускаем
	КонецПопытки;
	...
	РежимПоказа = 0;
	Попытка
		РежимПоказа = Форма.конс_РежимПоказаПеременных;
	Исключение
		// Реквизит отсутствует
	КонецПопытки;
	
	Если РежимПоказа = 1 Тогда
		ПоказатьПеременныеВТабло(Форма, Результат.ОписаниеПеременных, ИмяПоляHTML);
	ИначеЕсли РежимПоказа = 2 Тогда
...
Процедура ОбработатьУспешноеВыполнение(Форма, Результат, ИмяПоляHTML)
	...
	Если Форма.конс_РежимПоказаПеременных = 1 Тогда
		ПоказатьПеременныеВТабло(Форма, Результат.ОписаниеПеременных, ИмяПоляHTML);
		Возврат;
	КонецЕсли;
	
	Если Форма.конс_РежимПоказаПеременных = 2 Тогда
...
// Если передано явное значение - возвращает его.
// Иначе берет из реквизита формы конс_ИмяПоляHTML или возвращает "HTML".
//
Функция ПолучитьИмяПоляHTML(Форма, ИмяПоляHTML)
	
	Если ИмяПоляHTML <> Неопределено Тогда
		Возврат ИмяПоляHTML;
	КонецЕсли;
	
	Попытка
		РеквизитИмя = Форма.конс_ИмяПоляHTML;
...
// Извлекает позицию (строка, колонка) из описания ошибки.
// Формат: {Модуль(строка,колонка)}: Описание
//
Функция ИзвлечьПозициюИзОписанияКлиент(ОписаниеОшибки)
	
	Результат = Новый Структура("НомерСтроки, НомерКолонки", 0, 1);
	
	ПозицияСкобки = СтрНайти(ОписаниеОшибки, "(");
	Если ПозицияСкобки > 0 Тогда
		ПозицияЗакрытия = СтрНайти(ОписаниеОшибки, ")", , ПозицияСкобки);
		Если ПозицияЗакрытия > 0 Тогда
			Координаты = Сред(ОписаниеОшибки, ПозицияСкобки + 1, ПозицияЗакрытия - ПозицияСкобки - 1);
			Части = СтрРазделить(Координаты, ",");
			Если Части.Количество() >= 1 Тогда
				Результат.НомерСтроки = Макс(0, БезопасноеПреобразованиеВЧисло(Части[0], 0));
			КонецЕсли;
			Если Части.Количество() >= 2 Тогда
				Результат.НомерКолонки = Макс(1, БезопасноеПреобразованиеВЧисло(Части[1], 1));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
