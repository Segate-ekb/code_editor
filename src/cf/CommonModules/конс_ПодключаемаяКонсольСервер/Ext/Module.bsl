...
Функция ПрочитатьТекстМодуляОбъекта(КаталогИсходногоКода, ТипМодуля, КоллекцияМетаданных, ИмяОбъекта) Экспорт
	...
	// DEBUG
	// ЗаписьЖурналаРегистрации("DEBUG.ПрочитатьТекстМодуляОбъекта", , , , 
	// 	"Каталог=" + КаталогИсходногоКода + ", Тип=" + ТипМодуля + ", Коллекция=" + КоллекцияМетаданных + ", Объект=" + ИмяОбъекта);
	...
	Если ИнформацияОбъекта = Неопределено Тогда
		// ЗаписьЖурналаРегистрации("DEBUG.ПрочитатьТекстМодуляОбъекта", , , , "ИнформацияОбъекта = Неопределено");
		Возврат Неопределено;
	КонецЕсли;
	...
	// DEBUG
	// ЗаписьЖурналаРегистрации("DEBUG.ПрочитатьТекстМодуляОбъекта", , , , "ФайлМодуля=" + ФайлМодуля);
	
	// Читаем текст модуля
	ТекстМодуля = ПрочитатьТекстФайла(ФайлМодуля);
	Если ТекстМодуля = Неопределено Тогда
		// ЗаписьЖурналаРегистрации("DEBUG.ПрочитатьТекстМодуляОбъекта", , , , "Файл не найден или не прочитан");
		Возврат Неопределено;
	КонецЕсли;
	...
	// DEBUG
	// ЗаписьЖурналаРегистрации("DEBUG.ПрочитатьТекстМодуляОбъекта", , , , "АдресЗагрузки=" + АдресЗагрузки + ", длина текста=" + СтрДлина(ТекстМодуля));
...
//   РежимПоказаПеременных - Число - режим показа переменных (0/1/2)
//
// Возвращаемое значение:
...
Функция РазобратьИнформациюОбОшибкеСервер(ИнфоОшибки) Экспорт
	...
	ПозицияИзОписания = ИзвлечьПозициюИзОписания(ИнфоОшибки.Описание);
	...
	Если ИнфоОшибки.Причина <> Неопределено Тогда
		Результат.Вставить("Причина", РазобратьИнформациюОбОшибкеСервер(ИнфоОшибки.Причина));
	Иначе
...
//
// Возвращаемое значение:
//   Структура - НомерСтроки, НомерКолонки
//
Функция ИзвлечьПозициюИзОписания(ОписаниеОшибки)
