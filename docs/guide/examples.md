...
## Выполнение кода с процедурами и функциями (инлайнинг)

Оператор `Выполнить()` в 1С не поддерживает определения процедур и функций. Консоль решает эту проблему через **инлайнинг** — автоматическое преобразование кода с процедурами/функциями в плоский код.

### Возможности инлайнера

- Процедуры и функции с параметрами (`Знач`, значения по умолчанию)
- Вложенные вызовы: `Foo(Bar(1))`
- Множественные `Возврат` внутри функций
- Обнаружение рекурсии (ограничение глубины)
- Маркеры `//@@N` для сопоставления строк ошибок с исходным кодом

### Пример использования

```bsl
&НаКлиенте
Процедура ВыполнитьКодСИнлайнингом(Команда)

    // Получаем инлайненный код (процедуры/функции → плоский код)
    Код = конс_ПодключаемаяКонсольКлиент.ПолучитьИнлайнТекст(ЭтаФорма);
...
```bsl
Функция Удвоить(Знач Число)
    Возврат Число * 2;
КонецФункции

Функция Сумма(Знач А, Знач Б)
    Возврат А + Б;
КонецФункции

// Основное тело
Результат = Сумма(Удвоить(5), 10);
Сообщить(Результат);  // Выведет 20
```

После инлайнинга код преобразуется в плоский вид, который можно выполнить через `Выполнить()`.

### Ограничения

- Не поддерживается взаимная рекурсия
- Директивы компиляции (`&НаСервере`) игнорируются
- Препроцессорные инструкции (`#Если`) не обрабатываются внутри тел методов
