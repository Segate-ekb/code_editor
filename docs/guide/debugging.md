...
### Включение/отключение UI отладки

```bsl
// Включить панель отладки и нижнее табло
конс_ПодключаемаяКонсольКлиент.РежимОтладки(ЭтаФорма, Истина);

// Отключить панель отладки и нижнее табло
// (автоматически завершает активную сессию отладки)
конс_ПодключаемаяКонсольКлиент.РежимОтладки(ЭтаФорма, Ложь);
```
...
| `setDebugPanels(enabled)` | Включить/отключить весь UI отладки целиком: панель инструментов, glyph margin и нижнее табло (переменные, сообщения, ошибки, стек вызовов). При отключении также завершает активную сессию отладки |
